...
namespace VRTK {
    ...
    public sealed class VRTK_SDKManager : MonoBehaviour {
        ...
        public sealed class ScriptingDefineSymbolPredicateInfo {
            ...
            public ScriptingDefineSymbolPredicateInfo(SDK_ScriptingDefineSymbolPredicateAttribute attribute,
        ...
        public struct LoadedSetupChangeEventArgs {
            ...
            public LoadedSetupChangeEventArgs(VRTK_SDKSetup previousSetup, VRTK_SDKSetup currentSetup,
                string errorMessage) {
        ...
        /// </summary>
        public static ReadOnlyCollection<ScriptingDefineSymbolPredicateInfo>
            AvailableScriptingDefineSymbolPredicateInfos { get; private set; }
        ...
        /// <returns>Returns `true` if the SDK Manager instance was valid.</returns>
        public static bool AttemptTryLoadSDKSetup(int startIndex, bool tryToReinitialize,
            params VRTK_SDKSetup[] sdkSetups) {
            if (ValidInstance()) {
        ...
        [Tooltip(
            "Determines whether the scripting define symbols required by the installed SDKs are automatically added to and removed from the player settings.")]
        ...
        [Tooltip(
            "Determines whether the VR settings of the Player Settings are automatically adjusted to allow for all the used SDKs in the SDK Setups list below.")]
        public bool autoManageVRSettings = true;

        [Tooltip(
            "Determines whether the SDK Setups list below is used whenever the SDK Manager is enabled. The first loadable Setup is then loaded.")]
        ...
        [Obsolete(
            "`VRTK_SDKManager.persistOnLoad` has been deprecated and will be removed in a future version of VRTK. See https://github.com/thestonefox/VRTK/issues/1316 for details.")]
        [ObsoleteInspector]
        ...
        public bool ManageScriptingDefineSymbols(bool ignoreAutoManageScriptDefines, bool ignoreIsActiveAndEnabled) {
            if (!((ignoreAutoManageScriptDefines || autoManageScriptDefines) &&
            ...
            Dictionary<BuildTargetGroup, HashSet<string>> newSymbolsByTargetGroup =
                new Dictionary<BuildTargetGroup, HashSet<string>>(targetGroups.Length);
            ...
            foreach (BuildTargetGroup targetGroup in targetGroups) {
                IEnumerable<string> nonSDKSymbols = PlayerSettings.GetScriptingDefineSymbolsForGroup(targetGroup)
                    .Split(';')
                    .Where(symbol =>
                        !symbol.StartsWith(SDK_ScriptingDefineSymbolPredicateAttribute.RemovableSymbolPrefix,
            ...
            HashSet<string> activeSymbols =
                new HashSet<string>(
                    activeScriptingDefineSymbolsWithoutSDKClasses.Select(attribute => attribute.symbol));
            ...
            foreach (ScriptingDefineSymbolPredicateInfo predicateInfo in AvailableScriptingDefineSymbolPredicateInfos) {
                ...
                if (!activeSymbols.Contains(symbol)
                    && !descriptions.Any(description => description.symbol == symbol
                                                        && description.buildTargetGroup ==
                                                        predicateAttribute.buildTargetGroup)) {
                    continue;
                }
                ...
                SDK_ScriptingDefineSymbolPredicateAttribute[] allAttributes =
                    (SDK_ScriptingDefineSymbolPredicateAttribute[]) methodInfo.GetCustomAttributes(
                        typeof(SDK_ScriptingDefineSymbolPredicateAttribute), false);
            ...
            foreach (KeyValuePair<BuildTargetGroup, HashSet<string>> keyValuePair in newSymbolsByTargetGroup) {
                ...
                if (removedSymbols.Length > 0) {
                    VRTK_Logger.Info(VRTK_Logger.GetCommonMessage(
                        VRTK_Logger.CommonMessageKeys.SCRIPTING_DEFINE_SYMBOLS_REMOVED, targetGroup,
                ...
                if (addedSymbols.Length > 0) {
                    VRTK_Logger.Info(VRTK_Logger.GetCommonMessage(
                        VRTK_Logger.CommonMessageKeys.SCRIPTING_DEFINE_SYMBOLS_ADDED, targetGroup,
        ...
        public void TryLoadSDKSetup(int startIndex, bool tryToReinitialize, params VRTK_SDKSetup[] sdkSetups) {
            ...
            foreach (VRTK_SDKSetup invalidSetup in sdkSetups.Where(setup => !setup.isValid)) {
                ...
                if (!string.IsNullOrEmpty(setupErrorDescriptions)) {
                    ...
                    VRTK_Logger.Warn(string.Format(
                        "Ignoring SDK Setup '{0}' because there are some errors with it:\n{1}", invalidSetup.name,
            ...
            string loadedDeviceName =
                string.IsNullOrEmpty(XRSettings.loadedDeviceName) ? "None" : XRSettings.loadedDeviceName;
            ...
            if (!isDeviceAlreadyLoaded) {
                ...
                VRTK_SDKSetup[] missingVRDeviceSetups = sdkSetups
                    .Where(setup =>
                        setup.usedVRDeviceNames.Except(XRSettings.supportedDevices.Concat(new[] {"None"})).Any())
                    .ToArray();
                foreach (VRTK_SDKSetup missingVRDeviceSetup in missingVRDeviceSetups) {
                    ...
                    VRTK_Logger.Warn(string.Format(
                        "Ignoring SDK Setup '{0}' because the following VR device names are missing from the PlayerSettings:\n{1}",
        ...
        public void SetLoadedSDKSetupToPopulateObjectReferences(VRTK_SDKSetup setup) {
            if (EditorApplication.isPlaying) {
                VRTK_Logger.Fatal(
                    "The method SetLoadedSDKSetupToPopulateObjectReferences should not be used when the application is playing.");
                return;
            }
...
#pragma warning restore 618
        ...
        private IEnumerator FinishSDKSetupLoading(VRTK_SDKSetup[] sdkSetups, VRTK_SDKSetup previousLoadedSetup) {
            ...
            if (loadedSetup.usedVRDeviceNames.Except(new[] {"None"}).Any()) {
                ...
                if (!XRDevice.isPresent) {
                    ...
                    string errorMessage =
                        "An SDK Setup from the provided list could be loaded, but the device is not in working order.";
        ...
        private IEnumerator CheckLeftControllerReady() {
            ...
            while (true) {
                if (loadedSetup != null && loadedSetup.actualLeftController != null &&
                    loadedSetup.actualLeftController.activeInHierarchy &&
                    (loadedSetup.controllerSDK.GetCurrentControllerType() !=
                     SDK_BaseController.ControllerType.Undefined || maxCheckTime < 0)) {
                    break;
                }
        ...
        private IEnumerator CheckRightControllerReady() {
            ...
            int maxCheckTime = checkControllerValidTimer;
            while (true) {
                if (loadedSetup != null && loadedSetup.actualRightController != null &&
                    loadedSetup.actualRightController.activeInHierarchy &&
                    (loadedSetup.controllerSDK.GetCurrentControllerType() !=
                     SDK_BaseController.ControllerType.Undefined || maxCheckTime < 0)) {
                    break;
                }
        ...
        private void ToggleBehaviours(bool state) {
            ...
            for (int index = 0; index < listCopy.Count; index++) {
                ...
                if (behaviour == null) {
                    VRTK_Logger.Error(string.Format(
                        "A behaviour to toggle has been destroyed. Have you forgot the corresponding call `VRTK_SDKManager.instance.RemoveBehaviourToToggleOnLoadedSetupChange(this)` in the `OnDestroy` method of `{0}`?",
                        behaviour.GetType()));
                    _behavioursToToggleOnLoadedSetupChange.RemoveAt(state
                        ? index
                        : _behavioursToToggleOnLoadedSetupChange.Count - 1 - index);

                    continue;
                }

                behaviour.enabled = (state && _behavioursInitialState.ContainsKey(behaviour)
                    ? _behavioursInitialState[behaviour]
                    : state);
            }
        }
        ...
        private static void PopulateAvailableScriptingDefineSymbolPredicateInfos() {
            ...
            foreach (Type type in VRTK_SharedMethods.GetTypesOfType(typeof(VRTK_SDKManager))) {
                for (int index = 0;
                    index < type.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static).Length;
                    index++) {
                    MethodInfo methodInfo =
                        type.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static)[index];
                    SDK_ScriptingDefineSymbolPredicateAttribute[] predicateAttributes =
                        (SDK_ScriptingDefineSymbolPredicateAttribute[]) methodInfo.GetCustomAttributes(
                            typeof(SDK_ScriptingDefineSymbolPredicateAttribute), false);
                    ...
                    if (methodInfo.ReturnType != typeof(bool) || methodInfo.GetParameters().Length != 0) {
                        VRTK_Logger.Fatal(new InvalidOperationException(string.Format(
                            "The method '{0}' on '{1}' has '{2}' specified but its signature is wrong. The method must take no arguments and return bool.",
                            methodInfo.Name,
                            type,
                    ...
                    predicateInfos.AddRange(predicateAttributes.Select(predicateAttribute =>
                        new ScriptingDefineSymbolPredicateInfo(predicateAttribute, methodInfo)));
                }
            }

            predicateInfos.Sort((x, y) =>
                string.Compare(x.attribute.symbol, y.attribute.symbol, StringComparison.Ordinal));
        ...
        private static void PopulateAvailableAndInstalledSDKInfos() {
            ...
            PopulateAvailableAndInstalledSDKInfos<SDK_BaseSystem, SDK_FallbackSystem>(availableSystemSDKInfos,
                installedSystemSDKInfos, symbolsOfInstalledSDKs);
            PopulateAvailableAndInstalledSDKInfos<SDK_BaseBoundaries, SDK_FallbackBoundaries>(
                availableBoundariesSDKInfos, installedBoundariesSDKInfos, symbolsOfInstalledSDKs);
            PopulateAvailableAndInstalledSDKInfos<SDK_BaseHeadset, SDK_FallbackHeadset>(availableHeadsetSDKInfos,
                installedHeadsetSDKInfos, symbolsOfInstalledSDKs);
            PopulateAvailableAndInstalledSDKInfos<SDK_BaseController, SDK_FallbackController>(
                availableControllerSDKInfos, installedControllerSDKInfos, symbolsOfInstalledSDKs);
        ...
        private static void PopulateAvailableAndInstalledSDKInfos<BaseType, FallbackType>(
            List<VRTK_SDKInfo> availableSDKInfos,
            ...
            availableSDKInfos.AddRange(VRTK_SharedMethods.GetExportedTypesOfType(baseType)
                .Where(type =>
                    VRTK_SharedMethods.IsTypeSubclassOf(type, baseType) && type != fallbackType &&
                    !VRTK_SharedMethods.IsTypeAbstract(type))
        ...
        private static void RemoveLegacyScriptingDefineSymbols() {
            ...
            string[] newSymbols = currentSymbols
                .Where(symbol => !symbol.StartsWith("VRTK_SDK_", StringComparison.Ordinal)).ToArray();
            ...
            string[] removedSymbols = currentSymbols.Except(newSymbols).ToArray();
            if (removedSymbols.Length > 0) {
                VRTK_Logger.Info(string.Format(
                    "Legacy (i.e. used by previous VRTK versions only) Scripting Define Symbols removed from [Project Settings->Player] for {0}: {1}",
